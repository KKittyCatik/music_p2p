...
const docTemplate = `{
    ...
    "paths": {
        ...
        "/playback/play": {
            "post": {
                "description": "Records the given CID as the node's current track and marks playback active. State-only control-plane operation: it updates in-memory state (queryable via GET /playback/status) and does NOT spawn a P2P download or speaker output. A headless node has no audio device, so to actually listen, stream the audio over HTTP via GET /tracks/{cid}/stream instead.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
        ...
        "/playback/seek": {
            "post": {
                "description": "Records the given chunk index as the current-track position. State-only control-plane operation: it updates state queryable via GET /playback/status and does not reposition an in-flight HTTP audio stream.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
        ...
        "/playback/stop": {
            "post": {
                "description": "Clears the node's current-track state and marks playback inactive. State-only control-plane operation: like /playback/play it does not interrupt an in-flight HTTP audio stream (close the GET /tracks/{cid}/stream response on the client to stop listening).",
                "produces": [
                    "application/json"
                ],
                "tags": [
